...
> ⚠️ **免费档上必须另外配一个外部定时触发，否则数据不会更新。**
> 后端自己那个「每 12 小时」的定时任务是进程内的（`app/scheduler.py`），
> 只有同一个进程连续活满 12 小时才会触发。免费实例闲置 15 分钟就休眠，
> 进程根本活不到那么久，所以它**实际上从来不触发**——真实踩过：界面上
> 「上次更新」停在四天前，赔率覆盖掉到 2%。详见下面「定时更新」那一节。
...
配上 `ODDS_API_KEY` 之后，每轮更新会自动把 1X2 市场报价抓进来，预填到
「预测」页的赔率输入框，不用再一场场手打。不配这个变量，一切照旧走手动输入，
不会报错。
...
免费实例会休眠，进程内的定时任务指望不上（理由见上面 Render 那一节的警告）。
所以更新由**外部时钟**来敲：仓库里的 `.github/workflows/scheduled-update.yml`
每 12 小时打一次后端的 `POST /api/cron/update`。

### 要配的两个地方

1. **Render 环境变量**里加 `CRON_SECRET`，值是一串随机长字符串。
   例如用 `python3 -c "import secrets; print(secrets.token_urlsafe(32))"` 生成。
2. **GitHub 仓库** Settings → Secrets and variables → Actions 里加两个：

   | Secret | 填什么 |
   |---|---|
   | `CRON_SECRET` | 跟 Render 里那个**一模一样**的值 |
   | `BACKEND_URL` | 后端根地址，如 `https://xxx.onrender.com`（不带结尾斜杠、不带 `/api`） |

配好之后去 Actions 页面手动点一次「定时更新数据」→ Run workflow 验证。
日志里会打出 HTTP 码：`202` 是已启动，`200` 是「距上次更新不足 12 小时，
跳过」，`403` 是两边的密钥对不上，`503` 是 Render 那边没配 `CRON_SECRET`。

`CRON_SECRET` 没配时这个接口是**禁用**的（回 503），不是放行——不会因为
忘了配就把「让后端跑一轮全量更新」敞开给全网。

### 两个已知的脾气

- GitHub 的定时是**尽力而为**，延迟几分钟到几十分钟很常见。12 小时的节奏
  不受影响，但别指望它准点（界面上的「数据已过期」提示门槛设在 14 小时，
  就是为了给这个抖动留余量）。
- 仓库连续 60 天没有任何提交，GitHub 会自动停用定时工作流，要手动重新启用。

### 保活（可选，只影响打开速度）

上面那个定时触发保证的是**数据新鲜**。如果还想让网站**打开就快**（不用等
50 秒冷启动），可以用免费的外部监控（UptimeRobot、cron-job.org 之类）每
5-10 分钟 ping 一次 `GET /api/health`。这个接口是公开的，而且**不碰数据库**，
所以保活本身不产生任何 Supabase egress。

**不要用 GitHub Actions 做保活**——它的定时是尽力而为的，延迟经常超过
15 分钟，实例照样会睡着，时灵时不灵。

**实例小时数要算清楚。** Render 免费档是 750 小时/月，账号内所有免费服务
共享：

| 保活时段 | 每月实例小时 | 余量 |
|---|---|---|
| 24×7 全天 | 约 730 h | 约 20 h —— 几乎没有余量，再开一个免费服务就超额停服 |
| 每天 18 小时（如 08:00–02:00） | 约 547 h | 约 200 h，安全 |

推荐**按时段保活**：设成你实际会打开网站的时段。夜里那几小时让它睡，
不影响数据——数据由定时触发保证，跟实例醒不醒没关系。
...
1. 只给**未来 5 天内真的有比赛**的赛事发请求（`_ODDS_LOOKAHEAD_DAYS`），
   没有近期比赛的赛事一次配额都不花。
2. 一次请求覆盖该联赛全部近期比赛，不是一场一个请求。

按每天更新 2 次、同时有 3-4 个联赛在赛季中估算，大约 200-250 次/月，在免费档
之内。每轮更新的日志里会打印 `配额剩余 N`，可以直接盯这个数。

抓回来的价**不是 BK8 的价**，是全市场多家博彩公司的报价。界面上给两行：

- **平均价** —— 更接近你实际能成交的水平，用来预填输入框
- **最优价** —— 全市场最高报价，你在单一平台多半拿不到

两个都显示是有原因的：走查结论是热门-冷门偏差那条路子的盈亏**完全取决于
价格执行**（成交价 = 平均价 + f×(最优价−平均价)，f=0 时 ROI −3.54%，
f=0.8 才盈亏平衡）。这两个数的差距本身就是要看的信息，见 `handoff/09`。

下注前把输入框改成你在 BK8 实际看到的价——EV 和凯利用的是输入框里的数，
不是抓回来的数。
