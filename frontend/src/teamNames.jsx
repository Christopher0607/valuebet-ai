...
export const TEAM_ZH = {
  ...
  // 2026-27 新升班马（法甲勒芒、德甲埃尔沃斯贝格）。
  // 【2026-08-10 更新】原注释说它们"在参数表里没有历史数据、预测走兜底、
  // data_backing 标 none"——那是当时的事实，现在不是了：德乙/法乙已经
  // 并进俱乐部参数表（train_mle_club.py 的 TXT_LEAGUES），勒芒拿到 41 场、
  // 埃尔沃斯贝格 79 场真实数据，两支都是 full 了。注释留着记录这个变化。
  "Le Mans": "勒芒",

  // ── 2026-27 赛程里出现、但上面各块漏掉的 6 支（2026-08-09 补）──────
  // 上面的英甲/英乙/西乙块是按各联赛「最近一季名单」生成的，而 2026-27
  // 有升降级，名单换了人。这 6 支是拿 The Odds API 的真实赛程逐队跑
  // normalize_team_name 之后查出来的缺口，不是猜的。括号里是它们在
  // historical_results_club.csv 里的真实场次 + 参数表 attack 值，
  // 都有数据，不走兜底。
  "CD Leganés": "莱加内斯",              // 285 场, attack 0.0652
  "Granada CF": "格拉纳达",              // 365 场, attack 0.2289
  "Leicester City": "莱斯特城",          // 444 场, attack 0.4173
                                         // （上面英格兰联赛杯块里的
                                         //   "Leicester" 是另一套简称写法，
                                         //   两个 key 指向同一支队，不是重复）
